...
import { setClientTags } from "@/lib/actions/tags";
...
async function load(id: number) {
  ...
  return {
    ...
    client: { ...plain<ClientDoc>(client), tags: client.tags ?? [] },
...
export default async function ClientDetailPage({
  ...
  const [data, session, notes, tradeHistory, tagCatalogue] = await Promise.all([
    load(numId),
    getSession(),
  ...
  return (
    <div className="p-4 lg:p-5">
      <div className="mb-4">
        <Link href="/elefin/clients" className="text-[13px] text-accent hover:underline">
          ‹ Clients
        </Link>
        ...
        <div className="mt-2">
          <TagEditor
            ...
            onSave={async (tags) => {
              "use server";
              await setClientTags(numId, tags);
            }}
          />
        </div>
      </div>
              ...
              {tradeHistory.rows.length === 0 ? (
                ...
                tradeHistory.rows.map((t) => (
                  <tr key={t._id} className="border-b border-rule last:border-0 hover:bg-sunken">
                    ...
                    <td className="px-3 py-1.5">
                      <Link
                        href={`/elefin/accounts/${t.login}/history`}
                        className="font-mono text-[12px] text-accent hover:underline"
                      >
                        {t.login}
                      </Link>
...
function TPageLink({
  ...
  return (
    <Link
      href={`/elefin/clients/${clientId}${clientTradesQs({ ...q, page })}`}
