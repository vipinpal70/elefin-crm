...
export default async function ClientsPage({
  ...
  const [{ rows, total, codes, countries }, views] = await Promise.all([
    fetchClients(q),
  ...
  return (
    <div>
      ...
      <ClientsFilters q={q} codes={codes} countries={countries} total={total} />
...
function SortTh({
  ...
  return (
    ...
    >
      <Link
        href={`/clients${withParams(q, { sort: col, dir: nextDir })}`}
  ...
  return (
    <tr className="border-b border-rule hover:bg-sunken">
      <td className="px-3 py-1.5">
        <Link href={`/clients/${r._id}`} className="font-medium text-ink hover:text-accent">
          {r.name || `#${r._id}`}
        </Link>
...
function PageLink({
  ...
  return (
    <Link
      href={`/clients${withParams(q, { page })}`}
